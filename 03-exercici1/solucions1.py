...
def compta_vocals(text: str) -> int:
    ...
    """
    vocals = "aeiouAEIOU"
    return sum(1 for char in text if char in vocals)
...
def inverteix_diccionari(d: dict[str, int]) -> dict[int, str]:
    ...
    """
    return {valor: clau for clau, valor in d.items()}
...
def filtra_per_longitud(paraules: list[str], longitud_minima: int) -> list[str]:
    ...
    """
    return [paraula for paraula in paraules if len(paraula) >= longitud_minima]
...
def agrupa_per_paritat(numeros: list[int]) -> dict[str, list[int]]:
    ...
    """
    parells = [num for num in numeros if num % 2 == 0]
    senars = [num for num in numeros if num % 2 != 0]
    return {"parells": parells, "senars": senars}
...
def es_palindrom(text: str) -> bool:
    ...
    """
    text_net = text.replace(" ", "").lower()
    return text_net == text_net[::-1]
...
def crea_parelles(llista1: list[int], llista2: list[str]) -> list[tuple[int, str]]:
    ...
    """
    return list(zip(llista1, llista2))
